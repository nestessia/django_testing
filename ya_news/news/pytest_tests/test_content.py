...
def test_sorted_news(create_news):
    ...
    news_3 = create_news('News 3', 'Text 3')
    news_2 = create_news('News 2', 'Text 2')
    news_1 = create_news('News 1', 'Text 1')
    client = Client()
...
def test_sorted_comments(create_news):
    ...
    news = create_news('Test News', 'This is a test news')
    ...
    assert response.status_code == HTTPStatus.OK
    context = response.context['context']
    assert [context[0], context[1], context[2]] == [comment1, comment2, comment3]
'''
